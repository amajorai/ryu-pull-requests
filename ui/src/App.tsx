		...
		>
			<DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-xl">
				<DialogHeader>
					...
					<DialogDescription>
						Manage the dependency order of pull requests.
					</DialogDescription>
				</DialogHeader>
				{mode === "create" ? (
					<p>
				...
				<footer>
					...
					<Button
						...
						onClick={() => void submit()}
						type="button"
						variant={isUnstack ? "destructive" : "default"}
					>
		...
		<div className="detail-pane">
			<header className="detail-toolbar">
				...
				<div>
					...
					<DropdownMenu onOpenChange={setMergeOpen} open={mergeOpen}>
						<DropdownMenuTrigger
							render={<Button disabled={loading || detail.state !== "OPEN"} />}
						>
...
export function App() {
	const [view] = useState<AppView>(() => {
		const requested =
			typeof window === "undefined" ? undefined : window.ryu?.context?.view;
		return requested === "issues" ? "issues" : "pulls";
	});
	return (
		<main className="app-shell">
			{view === "pulls" ? <PullRequestInbox /> : <IssueInbox />}
		</main>
	);
}
